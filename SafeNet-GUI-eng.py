...
class SafeNetApp(ctk.CTk):
    def __init__(self):
        ...
        self.font_subtitle = ctk.CTkFont(family="Arial", size=15)
        ...
        self.subtitle_label = ctk.CTkLabel(self.main_frame, text="Smart content filtering system for families", font=self.font_subtitle)
        ...
        self.yt_label = ctk.CTkLabel(self.yt_frame, text="YouTube Hardening Mode:", font=self.font_bold)
        self.yt_label.pack(anchor="w", padx=15, pady=(10, 5))

        self.yt_var = ctk.StringVar(value="Strict (Recommended)")
        
        self.yt_segmented = ctk.CTkSegmentedButton(
            self.yt_frame, values=["Strict (Recommended)", "Moderate"], 
        ...
        self.install_btn = ctk.CTkButton(
            self.buttons_frame, text="Install / Update", font=self.font_bold, 
        ...
        self.status_btn = ctk.CTkButton(
            self.buttons_frame, text="Check Status", font=self.font_bold, 
        ...
        self.uninstall_btn = ctk.CTkButton(
            self.buttons_frame, text="Uninstall (Remove Filter)", font=self.font_bold, 
        ...
        self.output_box.insert("0.0", "Waiting for action...\n")
    ...
    def run_command(self, script_name, args=[], requires_admin=False):
        ...
        if requires_admin:
            self.log("\nNote: A separate Command Prompt window will open requesting Administrator privileges.")
            self.log("Please approve the request (Yes) and follow the instructions in the opened window.")

        script_dir = os.path.dirname(os.path.abspath(__file__))
        script_path = os.path.join(script_dir, script_name)
        
        if not os.path.exists(script_path):
            self.log(f"Error: File {script_name} not found.")
            return
